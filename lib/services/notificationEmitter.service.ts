...
/**
 * Run a post-commit notification block that cannot fail its caller.
 *
 * WHY THIS EXISTS, AND WHY emitQuietly IS NOT ENOUGH ON ITS OWN
 *   NotificationEmitterService promises never to throw, and it keeps that
 *   promise — but the promise covers the WRITE, not the recipient lookup that
 *   precedes it. Every emitting route reads its audience first:
 *
 *     await notificationEmitter.feeDemandGenerated({
 *       recipientUserIds: await findStudentUserIds(tenantId, studentIds),  // ← here
 *       …
 *     })
 *
 *   That inner await is a bare Prisma query. A pool timeout or a dropped
 *   connection there throws OUTSIDE the emitter's try/catch, lands in the
 *   route's own catch, and answers 500 — for work that is already committed.
 *
 *   On fee-demand generation that is not merely a misleading status. FeeDemand
 *   declares no unique constraint, so an operator who reads the 500 and retries
 *   bills the entire batch a second time. The failure mode is money, and its
 *   cause is a bell entry.
 *
 *   Wrapping the whole block — lookup and emission together — is what makes the
 *   emitter's stated contract true at the call site.
 *
 * WHAT THIS DELIBERATELY DOES NOT DO
 *   It does not retry, and it does not queue. A notification lost to a database
 *   blip stays lost, and the log line below is the only record of it. That is a
 *   real gap, and the honest fix is a transactional outbox — a row written in
 *   the same transaction as the business change and drained by a worker. This
 *   project has no worker and no queue infrastructure, so building half of one
 *   here would add a failure mode without closing this one. Recorded as a
 *   production follow-up instead.
 *
 * @param scope Route or operation identifier, so a swallowed failure is
 *              attributable in the log. Never interpolate user data into it.
 */
export async function notifyAfterCommit(
  ...
  try {
    ...
    // The business write already succeeded. This is logged rather than
    // rethrown, because the alternative is telling the caller their committed
    // work failed. Only the scope and the error are recorded — no recipient
    // ids, no request body, nothing that identifies a person.
    console.error(`[NotificationEmitter] post-commit emission failed in ${scope}`, err);
  }
}
