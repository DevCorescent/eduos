...
// ============================================================================
// notifyAfterCommit is the containment boundary for work that has ALREADY been
// committed. These assertions are the specification: if one of them changes,
// a route somewhere gained the ability to report a false failure for a write
// that succeeded — which on fee-demand generation means an operator retries and
// bills a batch twice.
// ============================================================================

describe("notifyAfterCommit", () => {
  it("swallows a throw from the recipient lookup, not just from the write", async () => {
    // The lookup sits OUTSIDE emitQuietly at every call site, which is exactly
    // the gap this helper closes.
    await notifyAfterCommit("TEST /lookup", async () => {
      throw new Error("pool timeout resolving recipients");
    });
    // Reaching here at all is the assertion: it did not rethrow.
    assert.ok(true);
  });

  it("swallows a rejection, not only a synchronous throw", async () => {
    await notifyAfterCommit("TEST /reject", () => Promise.reject(new Error("boom")));
    assert.ok(true);
  });

  it("runs the block — containment must not mean skipping the emission", async () => {
    let ran = false;
    await notifyAfterCommit("TEST /runs", async () => {
      ran = true;
    });
    assert.equal(ran, true);
  });

  it("resolves rather than returning a value, so no caller can branch on it", async () => {
    const result = await notifyAfterCommit("TEST /void", async () => {});
    assert.equal(result, undefined);
  });

  it("names its scope in the log so a swallowed failure stays attributable", async () => {
    ...
    assert.match(String(lines[0][0]), /POST \/api\/fee-demands\/generate/);
  });
});
