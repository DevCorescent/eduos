...
import { getApplication } from "@/services/admissions";
import { listTenantBatches, listTenantProgrammes } from "@/services/tenants";
import {
  ADMISSION_STAGES,
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const result = await getApplication(id, applicationId);
  return { title: result.success ? result.data.applicationNo : "Application" };
}

/**
 * PRD §8.2 / §49.2 — one application, its workflow position, and the two
 * actions that move it.
 *
 * The stage rail renders §49.2's twelve stages in the PRD's order, so an
 * operator can see where the applicant is and what remains — the whole point of
 * "Application status tracking".
 */
export default async function ApplicationDetailPage({ params }: { params: Params }) {
  const { id, applicationId } = await params;

  const [applicationResult, programmesResult, batchesResult] = await Promise.all([
    getApplication(id, applicationId),
    listTenantProgrammes(id),
    listTenantBatches(id),
  ]);

  // notFound() for a missing application; any other failure goes to the error
  // boundary. Conflating them would report an outage as a deleted record.
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="flex flex-col gap-6 lg:col-span-2">
          <ApplicationActions
            ...
            batches={batchesResult.success ? batchesResult.data.batches : []}
          />
