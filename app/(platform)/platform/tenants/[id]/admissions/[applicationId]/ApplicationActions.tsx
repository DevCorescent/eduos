...
export interface ApplicationActionsProps {
  ...
  /** Next stage per §49.2, or null when the application is at the last one. */
...
/**
 * The two actions that move an application: advance one §49.2 stage, and §8.5
 * conversion.
 *
 * BOTH ARE SERVER-DECIDED
 *   The advance button sends the next stage the SERVER computed and rendered;
 *   the service still re-reads the current stage and refuses anything that is
 *   not exactly one step on, so a stale page cannot skip a stage.
 *
 *   Conversion is offered only from STUDENT_ID_GENERATION onward — the §49.2
 *   stage at which a student identity comes into existence — and the service
...
export function ApplicationActions({
  ...
  const [programmeId, setProgrammeId] = useState(
    application.preferences[0]?.programme.id ?? ""
  );
  ...
  const canConvert =
    !converted &&
    ["STUDENT_ID_GENERATION", "COURSE_ALLOCATION", "PORTAL_ACTIVATION"].includes(
      application.stage
    );
  ...
  async function onAdvance() {
    ...
    setError(null);
    setIsAdvancing(true);
    const result = await advanceStage(tenantId, application.id, nextStage);
  ...
  async function onConvert() {
    ...
    const result = await convertApplication(tenantId, application.id, { programmeId, batchId });
    ...
    // The credential is shown once and nowhere else — the server stores only
    // its hash.
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          <Select
            ...
            // Defaulted to the applicant's first preference, but still a
            // decision: the PRD defines no rule for choosing between
            // preferences, so it is the officer's to make.
          ...
          <Input
            ...
            readOnly
            helperText="Becomes their sign-in address. Taken from the application."
          />
          <Alert variant="info">
            A temporary password is generated and shown once. They must change it before using
            the portal. Courses, fee plan and mentor are not assigned — those rules are not
            defined.
          </Alert>
        </div>
      </Modal>
