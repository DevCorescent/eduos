...
export async function POST(request: NextRequest) {
  try {
    ...
    // Wrapped rather than left bare: the two reads below are ordinary Prisma
    // queries, and a throw from either would answer 500 for a register that is
    // already written. The emitter's own guarantee covers its write, not the
    // lookups that feed it.
    await notifyAfterCommit("POST /api/attendance", async () => {
