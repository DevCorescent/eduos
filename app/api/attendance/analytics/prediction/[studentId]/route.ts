// ============================================================================
// OWNER  : Gauransh
// MODULE : Attendance Analytics — Prediction
...
//          STUDENT — their own prediction only.
// BACKEND: attendanceAnalyticsController -> attendanceAnalyticsService ->
//          attendanceAnalyticsRepository -> Prisma.
// PURPOSE: Classify a student's current attendance standing (SAFE / WARNING /
//          CRITICAL against the minimum requirement) and project their
//          percentage forward from their recent attendance trend.
//
// FIX (Phase 15 review): this route previously imported
// attendanceAnalyticsParamSchema from a "*.validation" path that does not
// exist, and enforced no self-scoping — a STUDENT caller could run a
// prediction against any studentId. Both fixed here, matching
...
// RESPONSE   : { success: true, data: AttendancePredictionDTO }
...
export async function GET(
  ...
  try {
    ...
    const prediction = await attendanceAnalyticsController.getPrediction(tenant.id, studentId);

    return NextResponse.json(ok(prediction, "Attendance prediction generated successfully."));
  } catch (error) {
    if (error instanceof AppError) {
      return NextResponse.json(fail(error.message, error.code), { status: error.statusCode });
    }

    console.error("[GET /api/attendance/analytics/prediction/[studentId]]", error);
